...
function testPlot(data, time, labels, testName, xMax)
    ...
    inCurLine  = line(time, data(:,2), 'Color',[0 .5 0]);
    curAxis = gca;
    set(curAxis, 'YAxisLocation', 'right');
    set(curAxis, 'XColor',        'black');
    set(curAxis, 'YColor',        'red');
    
    ax1_pos = get(curAxis,'Position');
    voltAxis = axes('Position',ax1_pos,...
                    'YAxisLocation','left',...
                    'Color','none');
    set(voltAxis, 'XTickLabelMode', 'Manual');  %remove the redundant ticks from x axis
    set(voltAxis, 'XTick', []);
    set(voltAxis, 'YColor', 'b');
