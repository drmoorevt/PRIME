...
%data3 = outputCurrent
%data4 = state
function testPlot(data, time, labels, testName, yMin, yMax)
    newFig = figure; % create a new figure to plot on
    set(newFig, 'Color', 'white')
    
    xMax = numel(data(:,1)) / 1000;
    ...
    voltAxis = axes('YAxisLocation',   'left',      ...
     ...
     iCurAxis = axes('YAxisLocation',  'right',     ...
                     ...
                     'YColor',          [0 .5 0],   ...
                     'YLim',            [yMin, yMax], ...
                     'YTick',           [yMin : yMin + (yMax - yMin)/8 : yMax], ...
                     'YTickMode',       'manual');
    ...
    stateAxis = axes('YAxisLocation',  'left',      ...
                    'Color',          'none',      ...
                    'XLim',           [0, xMax], ...
                    'XColor',         'black',     ...
                    'XTickLabelMode',   'Manual',    ... % remove xticks
    ...
    set(stateAxis, 'Position', get(stateAxis, 'Position') + [-.04, 0, +.04, 0])
    
    linkaxes([oCurAxis, iCurAxis], 'xy') % link the y axes of output currents
    linkaxes([voltAxis, stateAxis],'xy') % link the y axes of the state and voltage
    linkaxes([iCurAxis, oCurAxis, voltAxis, stateAxis],'x') % link the x axes for proper zooming
    lnkObj = linkprop([voltAxis stateAxis], 'YLim');
    ...
    stateLine  = line(time, data(:,4),'Parent',stateAxis,'Color','black','LineWidth',3);
    
    set(gca,'YTickLabel',[]);  % now hide the tick labels because linkaxes doesn't seem to be fixing zoom properly
    return
